...
int AVIFile::GetFrameInfo( off_t &offset, int &size, int frameNum )
{
	switch ( index_type )
	{
...
int AVIFile::GetFrame( Frame *frame, int frameNum )
{
	off_t	offset;
	int	size;

	if ( GetFrameInfo( offset, size, frameNum ) != 0 )
		return -1;
...
bool AVI1File::WriteFrame( Frame *frame )
{
	int frame_chunk;
	//    int         junk_chunk;
	int num_blocks;
...
bool AVI2File::WriteFrame( Frame *frame )
{
	int audio_chunk;
	int frame_chunk;
	//    int         junk_chunk;
